...
func CreateBridge(w http.ResponseWriter, r *http.Request, rdb *redis.Client, ctx context.Context) {
	...
	if req.Playground == "" {
		sendErrorResponse(w, "Playground name is required", http.StatusBadRequest)
		return
	}

	nid, cid, ip, subnet, gateway, err := creator.CreateDockerBridge(req.Name, req.Image, req.Playground)
	if err != nil {
